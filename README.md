...
返回内容包括总检查项、已响应项、待补强项、未响应项、高风险未响应数量、明细列表和处理建议。该指标在界面中命名为“条款响应覆盖率”，用于追踪招标条款、评分项、风险项是否被当前章节映射或正文片段承接，不等同于最终 Word 标书合规结论。标书工作台顶部会常驻显示实时质量仪表盘，并在进入工作台、生成正文、批量生成、保存章节和切换分册后刷新条款响应报告；若下载前仍存在未响应或高风险未响应项，会先弹窗提示风险，再由用户决定继续下载或返回补强。
